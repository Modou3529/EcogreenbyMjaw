...
// Apply on load — dark mode is the default
(function() {
...
function updateKeyStatus(){
  ...
  if(geminiKey){
    ...
    bannerSub.textContent='Powered by Google Gemini 2.5 Flash · Click to change key';
...
// ===== GEMINI API =====
async function callGemini(prompt,systemInstruction='',forceJson=false){
  if(!geminiKey){openApiModal();throw new Error('No API key.');}
  const genConfig={maxOutputTokens:1200,temperature:0.2};
  if(forceJson) genConfig.response_mime_type='application/json';
  const body={
...
function extractJSON(rawString){
  try{
    ...
    if(start!==-1&&end!==0){
      const snippet=rawString.slice(start,end);
...
async function generateIdeas(){
  ...
  try{
    const res=await callGemini(prompt,'You are a friendly engineering project advisor. Return only valid JSON.',true);
